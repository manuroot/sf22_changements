 ...
 * @ORM\Table(name="wdcalendar_fichiers")
 * @ORM\Entity(repositoryClass="Application\CalendarBundle\Repository\DocCalendarRepository")
...
class DocCalendar {
    ...
    /**
     *
     * @var ArrayCollection Projet $idcalendar
     *
     * Inverse Side
     * 
     * @ORM\ManyToMany(targetEntity="AdesignCalendar", mappedBy="picture",cascade={"persist"})
     */
     ...
     */
    protected $createdAt;
    protected $disk_path = 'uploads/calendars';
    ...
    public function __construct() {
        //   parent::__construct();
        $this->idcalendar = new ArrayCollection();
     ...
     * Add idcalendar
     *
     * @param \Application\CalendarBundle\Entity\AdesignCalendar $idcalendar
     * @return DocCalendar
     */
    public function addIdcalendar(AdesignCalendar $idcalendar) {
        if (!$this->idcalendar->contains($idcalendar)) {
            if (!$idcalendar->getPicture()->contains($this)) {

                $idcalendar->addPicture($this);  // Lie le Client au produit.
            }
            $this->idcalendar->add($idcalendar);
        }
        //$this->idcalendar[] = $idcalendar;
        //  return $this;
    }

    public function setIdcalendar($items) {
        if ($items instanceof ArrayCollection || is_array($items)) {
            foreach ($items as $item) {
                $this->addIdcalendar($item);
            }
        } elseif ($items instanceof Changements) {
            $this->addPicture($items);
        } else {
            throw new \Exception("$items must be an instance of Calendar or ArrayCollection");
        }
    }
     ...
     * @param \Application\CalendarBundle\Entity\AdesignCalendar $idcalendar
     */
    public function removeIdcalendar(AdesignCalendar $idcalendar) {
        if (!$this->idcalendar->contains($idcalendar)) {
            return;
        }
        $this->idcalendar->removeElement($idcalendar);
        $idcalendar->removePicture($this);
    }
     ...
     * 
     */
    public function setDiskPath($disk_path = 'uploads/calendars') {
